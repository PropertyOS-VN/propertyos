...
| `apps/billing-service` | Sinh hóa đơn định kỳ, tính tiền điện/nước/dịch vụ, báo cáo tài chính | Java 17 + Spring Boot 4 | Render (free, không cần thẻ) |
...
Next.js Route Handlers/Server Actions trên Vercel free tier bị giới hạn ~10s/request và không phù hợp cho job chạy nền dài hay tính toán nặng lặp lại theo lịch (VD: sinh hóa đơn cho toàn bộ hợp đồng vào đầu tháng). Những phần đó được tách sang `billing-service` (Java, Spring Boot) chạy trên Render — không giới hạn 10s, có thể dùng Spring Scheduler để chạy job định kỳ, và tận dụng Spring Data JPA cho các phép tính tài chính cần transaction chặt chẽ. Container free tier của Render sleep sau 15 phút không traffic (cold start ~30-50s khi có request/job đầu tiên đánh thức) — chấp nhận được vì đổi lại không cần thẻ tín dụng, không có rủi ro phát sinh phí.
...
└──────────┬───────────────────┘
           ...
           ▼
┌─────────────────────────────┐
│  apps/billing-service        │   Render (free, không cần thẻ)
│  (Spring Boot)                │
...
`docker-compose.yml` ở root chỉ chạy Postgres + MongoDB cho môi trường dev (mô phỏng Supabase/Atlas cục bộ, không cần internet khi code). `billing-service` có Dockerfile riêng để test build trước khi deploy Render.
