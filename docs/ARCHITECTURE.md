...
|---|---|---|---|
| `apps/web-app` | FE + BE nhẹ: đăng nhập, quản lý tòa nhà/phòng, hợp đồng, xem hóa đơn | Next.js (App Router) + React + Tailwind + Framer Motion + HeroUI Pro | Vercel (free) |
| `apps/billing-service` | Sinh hóa đơn định kỳ, tính tiền điện/nước/dịch vụ, báo cáo tài chính | Java 21 + Spring Boot | Google Cloud Run (free tier) |
...
```
┌─────────────────────────────┐
│   apps/web-app (Next.js)     │   Vercel (free)
│   - UI (HeroUI Pro)          │
│   - Auth (Supabase Auth)     │
│   - CRUD: building/room/     │
...
└───────────────┘
        ...
        │
   apps/web-app cũng đọc/ghi thẳng vào Postgres
   cho các thao tác CRUD đơn giản (không qua billing-service)
```

## 4. Xác thực & phân quyền

Dùng thẳng **Supabase Auth** (JWT built-in, hỗ trợ email/password + OAuth) — không tự viết auth-service nữa. Vai trò (`OWNER`, `MANAGER`, `ACCOUNTANT`) lưu ở bảng `profiles` (Postgres), liên kết 1-1 với `auth.users` của Supabase. `billing-service` verify JWT của Supabase (JWKS endpoint) để xác thực request gọi từ `web-app`.
...
Nếu traffic thật tăng và cần tách `billing-service` thành nhiều service nhỏ hơn (VD: tách riêng phần báo cáo), hoặc cần message queue thật (Kafka/RabbitMQ) khi khối lượng event lớn, kiến trúc hiện tại vẫn dễ mở rộng vì đã tách rõ theo domain (`web-app` vs `billing-service`) và dùng Postgres/Mongo chuẩn (không khoá vào Supabase/Atlas cụ thể, có thể tự host lại khi cần).
