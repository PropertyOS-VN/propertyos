...
Cả `apps/admin-app` (Next.js, qua Supabase JS client) và `apps/billing-service` (Spring Boot, qua JDBC + Spring Data MongoDB) đều kết nối vào cùng 2 DB này — không còn database-per-service như bản thiết kế cũ, vì quy mô hiện tại (dự án cá nhân) không cần cách ly DB theo service.
...
**invoices** (ghi bởi `billing-service`, đọc bởi `admin-app`)
| Cột | Kiểu | Ghi chú |
|---|---|---|
| id | bigint PK (identity) | |
...
Bật **Row Level Security (RLS)** trên toàn bộ bảng — vì `admin-app` gọi thẳng Supabase từ client, RLS là lớp bảo vệ bắt buộc (VD: Manager chỉ thấy building mình quản lý). `billing-service` dùng service-role key (bypass RLS) vì chạy phía server tin cậy.
...
**room_attributes** — thuộc tính linh hoạt theo loại hình bất động sản, do `admin-app` đọc/ghi trực tiếp:
```json
{
  "room_id": 101,
